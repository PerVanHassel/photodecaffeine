...
export interface QueryState<T> {
  data: T | undefined;
  loading: boolean;
  /** A refresh over data already on screen — never blanks the list. */
  refreshing: boolean;
  error: string | null;
  refresh: () => Promise<void>;
  /** Optimistic local updates, so a mutation needs no full round-trip. */
 ...
 * you (an aanvraag arrives, a colleague files a declaration), so every screen
 * entry re-reads instead of showing something that was true a while ago.
 */
export function useQuery<T>(fn: () => Promise<T>, deps: unknown[] = []): QueryState<T> {
  ...
  // overwriting newer data, and against setState on an unmounted screen.
